...
Vineapple.prototype = {
  ...
  login: function (username, password, callback) {
    ...
    request = this.request({
      ...
      form: {
        deviceToken: Vineapple.DEVICE_TOKEN || createDeviceToken(username, password),
        username: username,
        password: password
      }
    });
 ...
 */
function createDeviceToken (username, password) {
  var sha256 = crypto.createHash('sha256');
  var uniqueToken = [Vineapple.DEVICE_TOKEN_SEED, username, password].join(':');
  sha256.update(uniqueToken);
